...
public class Analyzer {
    ...
    private static int getMaxDimensionTraversal(ConstraintWidget constraintWidget, int i, boolean z, int i2) {
        ...
        if (constraintWidget.mOptimizerMeasurable) {
            ...
            while (true) {
                ...
                i13 = Math.max(i6, getMaxDimensionTraversal(((ResolutionAnchor) it.next()).myAnchor.mOwner, i, z, i12));
            }
            ...
            while (true) {
                ...
                i15 = Math.max(i7, getMaxDimensionTraversal(((ResolutionAnchor) it2.next()).myAnchor.mOwner, i, z, width2 + i12));
            }
            if (z2) {
                i8 = i6 - height;
            ...
            if (i == 1) {
                ...
                while (it3.hasNext()) {
                    ...
                    i17 = i5 == 1 ? Math.max(i17, getMaxDimensionTraversal(resolutionAnchor.myAnchor.mOwner, i, z, height + i12)) : Math.max(i17, getMaxDimensionTraversal(resolutionAnchor.myAnchor.mOwner, i, z, (baselineDistance * i5) + i12));
                }
                width2 = width2;
                i16 = (constraintWidget.mBaseline.getResolutionNode().dependents.size() <= 0 || z2) ? i17 : i5 == 1 ? i17 + height : i17 - baselineDistance;
            }
            int max = Math.max(i8, Math.max(width, i16));
            int i18 = i11 + margin;
            int i19 = i18 + width2;
            if (i5 == -1) {
                i9 = i19;
            ...
            if (z) {
                Optimizer.setOptimizedWidget(constraintWidget, i, i9);
                constraintWidget.setFrame(i9, i19, i);
            } else {
                constraintWidget.mBelongingGroup.addWidgetsToSet(constraintWidget, i);
                constraintWidget.setRelativePositioning(i9, i);
            }
            ...
            if (constraintWidget.mListAnchors[i4].mTarget != null && constraintWidget.mListAnchors[i14].mTarget != null && constraintWidget.mListAnchors[i4].mTarget.mOwner == (parent = constraintWidget.getParent()) && constraintWidget.mListAnchors[i14].mTarget.mOwner == parent) {
                constraintWidget.mBelongingGroup.addWidgetsToSet(constraintWidget, i);
            }
            return margin + max;
        }
    ...
    private static int resolveDimensionRatio(ConstraintWidget constraintWidget) {
        ...
        if (constraintWidget.getHorizontalDimensionBehaviour() == ConstraintWidget.DimensionBehaviour.MATCH_CONSTRAINT) {
            i = constraintWidget.mDimensionRatioSide == 0 ? (int) (constraintWidget.getHeight() * constraintWidget.mDimensionRatio) : (int) (constraintWidget.getHeight() / constraintWidget.mDimensionRatio);
            ...
            i = constraintWidget.mDimensionRatioSide == 1 ? (int) (constraintWidget.getWidth() * constraintWidget.mDimensionRatio) : (int) (constraintWidget.getWidth() / constraintWidget.mDimensionRatio);

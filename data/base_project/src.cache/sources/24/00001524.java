...
/* loaded from: InputEventConsistencyVerifier.class */
public final class InputEventConsistencyVerifier {
                ...
                case 0:
                    ...
                    if (state != null) {
                        if (state.unhandled) {
                            ...
                            break;
                        } else if ((this.mFlags & 1) == 0 && event.getRepeatCount() == 0) {
                    ...
                    default:
                        ...
                        if (actionMasked == 5) {
                            ...
                            if (actionIndex < 0 || actionIndex >= pointerCount) {
                                problem("ACTION_POINTER_DOWN index is " + actionIndex + " but the pointer count is " + pointerCount + Separators.DOT);
                                this.mTouchEventStreamIsTainted = true;
                            } else {
                            ...
                            if (actionIndex < 0 || actionIndex >= pointerCount) {
                                problem("ACTION_POINTER_UP index is " + actionIndex + " but the pointer count is " + pointerCount + Separators.DOT);
                                this.mTouchEventStreamIsTainted = true;
                            } else {
                                ...
                                if ((this.mTouchEventStreamPointers & idBit2) == 0) {
                                    ...
                                    this.mTouchEventStreamPointers &= idBit2 ^ (-1);
                                }
                            }
                            ensureHistorySizeIsZeroForThisAction(event);
                            break;
        ...
        if (this.mViolationMessage != null && this.mViolationMessage.length() != 0) {
            if (!this.mCurrentEvent.isTainted()) {
                ...
                if (this.mRecentEvents != null) {
                    this.mViolationMessage.append("\n  -- recent events --");
                    for (int i = 0; i < 5 && (event = this.mRecentEvents[(index = ((this.mMostRecentEventIndex + 5) - i) % 5)]) != null; i++) {
                        this.mViolationMessage.append("\n  ");
    ...
    /* loaded from: InputEventConsistencyVerifier$KeyState.class */

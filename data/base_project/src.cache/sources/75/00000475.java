...
/* loaded from: FilterFactory.class */
public class FilterFactory {
    ...
    public Filter createFilterByClassName(String className, String filterName) {
        ...
        while (i$.hasNext()) {
            ...
            try {
                if (mLogVerbose) {
                    Log.v(TAG, "Trying " + packageName + Separators.DOT + className);
                }
                synchronized (mClassLoaderGuard) {
                    filterClass = mCurrentClassLoader.loadClass(packageName + Separators.DOT + className);
                }

...
/* loaded from: BackupHelperDispatcher.class */
public class BackupHelperDispatcher {
    ...
    /* loaded from: BackupHelperDispatcher$Header.class */
    ...
    public void performBackup(ParcelFileDescriptor oldState, BackupDataOutput data, ParcelFileDescriptor newState) throws IOException {
        ...
        for (Map.Entry<String, BackupHelper> entry : helpers.entrySet()) {
            ...
            Log.d(TAG, "handling new helper '" + header.keyPrefix + Separators.QUOTE);
    ...
    private void doOneBackup(ParcelFileDescriptor oldState, BackupDataOutput data, ParcelFileDescriptor newState, Header header, BackupHelper helper) throws IOException {
        ...
        if (pos < 0) {
            throw new IOException("allocateHeader_native failed (error " + pos + Separators.RPAREN);
        }
        data.setKeyPrefix(header.keyPrefix);
        helper.performBackup(oldState, data, newState);
        int err = writeHeader_native(header, newStateFD, pos);
        if (err != 0) {
            throw new IOException("writeHeader_native failed (error " + err + Separators.RPAREN);
        }
    }
        ...
        while (input.readNextHeader()) {
            ...
            if (pos > 0) {
                ...
                if (helper != null) {
                    ...
                    Log.w(TAG, "Couldn't find helper for: '" + rawKey + Separators.QUOTE);
                    alreadyComplained = true;
                }
            } else if (!alreadyComplained) {
                Log.w(TAG, "Entity with no prefix: '" + rawKey + Separators.QUOTE);
                alreadyComplained = true;
            }

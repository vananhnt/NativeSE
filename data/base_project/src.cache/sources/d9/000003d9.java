...
public final class StringBlock {
    ...
    /* JADX INFO: Access modifiers changed from: package-private */
    /* loaded from: StringBlock$StyleIDs.class */
        ...
        for (int i = 0; i < style.length; i += 3) {
            ...
            if (type == ids.boldId) {
                ...
                if (tag.startsWith("font;")) {
                    ...
                    int len = tag.length();
                    int indexOf = tag.indexOf(59);
                    while (true) {
                        int t = indexOf;
                        if (t < len && (eq = tag.indexOf(61, t)) >= 0) {
    ...
    private static CharacterStyle getColor(String color, boolean foreground) {
        ...
        if (!TextUtils.isEmpty(color)) {
            if (color.startsWith(Separators.AT)) {
    ...
    private static void addParagraphSpan(Spannable buffer, Object what, int start, int end) {
        ...
        if (end != 0 && end != len && buffer.charAt(end - 1) != '\n') {
            while (true) {
                ...
                if (end >= len || buffer.charAt(end - 1) == '\n') {
                    break;
                }
            }
        }
    ...
    /* loaded from: StringBlock$Height.class */

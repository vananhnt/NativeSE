...
public class YuvImage {
    ...
    int[] calculateOffsets(int left, int top) {
        if (this.mFormat == 17) {
            int[] offsets = {(top * this.mStrides[0]) + left, (this.mHeight * this.mStrides[0]) + ((top / 2) * this.mStrides[1]) + ((left / 2) * 2)};
            return offsets;
        } else if (this.mFormat != 20) {
            return null;
        } else {
            int[] offsets2 = {(top * this.mStrides[0]) + ((left / 2) * 4)};
            return offsets2;
        }
    }

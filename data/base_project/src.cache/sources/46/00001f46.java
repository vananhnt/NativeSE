...
/* loaded from: PackageSignatures.class */
class PackageSignatures {
    ...
    /* JADX INFO: Access modifiers changed from: package-private */
    public void writeXml(XmlSerializer serializer, String tagName, ArrayList<Signature> pastSignatures) throws IOException {
        if (this.mSignatures == null) {
            return;
        }
        serializer.startTag(null, tagName);
        serializer.attribute(null, HardwareRenderer.OVERDRAW_PROPERTY_COUNT, Integer.toString(this.mSignatures.length));
        for (int i = 0; i < this.mSignatures.length; i++) {
            serializer.startTag(null, "cert");
            Signature sig = this.mSignatures[i];
    ...
    /* JADX INFO: Access modifiers changed from: package-private */
    public void readXml(XmlPullParser parser, ArrayList<Signature> pastSignatures) throws IOException, XmlPullParserException {
        String countStr = parser.getAttributeValue(null, HardwareRenderer.OVERDRAW_PROPERTY_COUNT);
        if (countStr == null) {
                            ...
                            try {
                                ...
                                if (key == null) {
                                    if (idx >= 0 && idx < pastSignatures.size()) {
                                        if (pastSignatures.get(idx) != null) {
                                            this.mSignatures[pos] = pastSignatures.get(idx);
                                            pos++;
                                        } else {
    ...
    /* JADX INFO: Access modifiers changed from: package-private */
    public void assignSignatures(Signature[] sigs) {
        if (sigs == null) {

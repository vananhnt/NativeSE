...
public class FilterGraph {
    ...
    private void runTypeCheckOn(Filter filter) {
        for (InputPort inputPort : filter.getInputPorts()) {
            ...
            if (sourceFormat != null && targetFormat != null) {
                if (this.mLogVerbose) {
                    Log.v(this.TAG, "Checking " + sourceFormat + " against " + targetFormat + Separators.DOT);
                }
    ...
    private void discardUnconnectedOutputs() {
        ...
        Iterator i$2 = addedFilters.iterator();
        while (i$2.hasNext()) {
            addFilter(i$2.next());
        }
    }
        ...
        for (Map.Entry<OutputPort, LinkedList<InputPort>> connection : this.mPreconnections.entrySet()) {
            ...
            if (inputPorts.size() == 1) {
                ...
                if (this.mAutoBranchMode == 1) {
                    int i = branchId;
                    branchId++;
                    FrameBranch branch = new FrameBranch("branch" + i);
                    new KeyValueMap();

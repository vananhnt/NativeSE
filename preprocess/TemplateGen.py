...
class TemplateGen:
    ...
    HYBRIDSE_DIR = '/home/va/git/HybridSE_2'
    ...
    def __find_native(self, folder) -> str:
        for native_file in Path(folder).rglob("*.so"):
            return native_file.parent.name + '/' + native_file.name
    ...
    def run_corana(self, sample_dir, class_name, package_name, entry, args_count=None, args_types=None):
        try:
    ...
    def run_corana_service(self, sample_dir, class_name, package_name, entry, args_count=None, args_types=None):
        try:
            java_file=sample_dir + '/src/' + package_name.replace('.', '/') + '/' + class_name + 'Wrapper.java'
            f_wrapper = open(java_file, "w")    
    ...
    def runActivities(self, sample_dir:str):
        outputs=[]
        ...
        for act in self.activities:
            outputs.append(self.run_corana(sample_dir, act, self.activities[act], 'onCreate', args_count=1, args_types=None))
            
        for ser in self.services:
            outputs.append(self.run_corana_service(sample_dir, ser, self.services[ser], 'onStart', args_count=0, args_types=None))
            outputs.append(self.run_corana_service(sample_dir, ser, self.services[ser], 'onCreate', args_count=0, args_types=None))
...
if __name__ == "__main__":            
    ...
    TemplateGen.build()
    #generator.save_csv('./temp/java_projects/towelroot.apk')
    # gen.runActivities('/home/va/git/APK-analyser/temp/java_projects_01/42d758ba933ad52fe0b5a719c0d88c9040372811d53f585a931a00b15176d1cf')
    # print(gen.stat)

...
public class Emulator {
    ...
    public void markSource(String sourceMethod) {
        currentSource = sourceMethod;
        env.markedSource = sourceMethod;
    }
    ...
    public BitVec add(BitVec a, BitVec b, boolean... carry) {
        ...
        boolean isTaint = a.taint || b.taint;
        return new BitVec(symVal, conVal, isTaint);
    }
    ...
    public void pop(char r) {
        ...
        BitVec popMem = Memory.get(env.register.get('s'));
    ...
    public void write(char r, BitVec value) {
        env.register.set(r, value);
        if (value != null && value.taint)
            Logs.infoLn("+++ Taint r" + r + " " + env.register.get(r).getSym());
    }

    public void taint(char r, BitVec prev) {
        env.register.get(r).taint();
        env.register.get(r).setPrev(prev);
        Logs.infoLn("+++ Taint r" + r + " " + env.register.get(r).getSym());
    }
    ...
    public void sanitize(char r) {
        if (env.register.get(r).taint)  {
            Logs.infoLn("+++ Clear r" + r);
        }
        env.register.get(r).clear();
    }
    //TODO: 17/01/2024 TEMPORARY
    public void callUnknownJNIEnv() {
        //r0 is *JNIENv -> only taint parameters difer from JNIEnv
        String JNIEnv = env.register.get('0').getSym();
        if (env.register.get('3').taint) {
            //System.out.println("+++ Taint parameters 1-3");
            env.register.get('0').taint();
            env.register.get('1').taint();
            env.register.get('2').taint();
            if (!env.register.get('1').getSym().equals(JNIEnv)) {
                updateTaintMemory(env.register.get('1').getSym());
            }
            if (!env.register.get('2').getSym().equals(JNIEnv)) {
                updateTaintMemory(env.register.get('2').getSym());
            }

        } else if (env.register.get('2').taint) {
            env.register.get('0').taint();
            env.register.get('1').taint();
            if (!env.register.get('1').getSym().equals(JNIEnv)) {
                updateTaintMemory(env.register.get('1').getSym());
            }
        } else if (env.register.get('1').taint) {
            env.register.get('0').taint();
        } else if (!env.register.get('0').taint && !env.register.get('1').taint && !env.register.get('2').taint ) {
            env.register.get('0').clear();
        }
    }

    private void updateTaintMemory(String address) {
        SortedSet<Character> keys = new TreeSet<>(env.register.regs.keySet());
        if (env.register.get('0').getSym().equals("address")) return;
        for (Character key : keys) {
            if (env.register.regs.get(key) instanceof BitVec) {
                BitVec val = env.register.get(key);
                if (val.getSym().equals(address) && (val.getSym() != "#x00000000") && (val.getSym() != env.register.get('0').getSym())) {
                    env.register.get(key).taint();
                }
            }

        }
    }
    public boolean isTaint(char r) {
        return env.register.get(r).taint;
    }
    ...
    public void ldr(Character xt, BitVec label) {
        ...
        if (label.taint) {
            ...
            //if (!result.equals("ERROR")) Memory.taint(SysUtils.getAddressValue(result));
        } else {
            sanitize(xt);
            //if (!result.equals("ERROR")) Memory.sanitize(SysUtils.getAddressValue(result));
        }
    }
    public void adr(Character xt, BitVec label) {
        arithmeticMode = ArithmeticMode.BINARY;
        //Check if the label is an address or arihmetic then get result of the bitvector arithmetic
        String result = (label.getSym().matches("[01][01]+") || label.getSym().contains("#x") || label.getSym().contains("SYM") ) ?
                label.getSym() : Z3Solver.solveBitVecArithmetic(label.getSym());
    ...
    protected void load(Character d, Character label) {
        ...
        //if (res.taint) System.out.println("Taint r" + d + " from " + res.getSym());
    }
    ...
    public void call(String jmpAddress) throws  Exception {
        String func = ExternalCall.findFunctionName(jmpAddress);
        if (func.equals("")) {
            func = DBDriver.getFunctionLabel(SysUtils.getAddressValue(jmpAddress));
        }
        exec(func);
    }

    public void exec(String func) throws Exception {
        // try {
...
//                APIStub.sigemptyset(env);         }
            ...
            APIStub.ynl(env);
        } else if ("pthread_cond_wait".equals(func)) {
            APIStub.pthread_cond_wait(env);
        } else if ("pthread_join".equals(func)) {
            APIStub.pthread_join(env);
        } else if ("pthread_mutex_unlock".equals(func)) {
            APIStub.pthread_mutex_unlock(env);
        } else if ("pthread_create".equals(func)) {
            APIStub.pthread_create(env);
        } else if ("pthread_mutex_lock".equals(func)) {
            APIStub.pthread_mutex_lock(env);
        } else if ("__android_log_print".equals(func)) {
            // UNSupported functions
            APIStub.__android_log_print(env);
        } else if ("_ZN7_JNIEnv9NewObjectEP7_jclassP10_jmethodIDz".equals(func)) {
            APIStub._ZN7_JNIEnv9NewObjectEP7_jclassP10_jmethodIDz(env);
        } else if ("_ZN7_JNIEnv16CallObjectMethodEP8_jobjectP10_jmethodIDz".equals(func)) {
            APIStub._ZN7_JNIEnv16CallObjectMethodEP8_jobjectP10_jmethodIDz(env);
        } else if ("JNIEnv_GetObjectArrayElement".equals(func)) {
            APIStub.JNIEnv_GetObjectArrayElement(env);
        }
//        } catch (Throwable e){

...
import main.corana.external.jni.*;
...
//    }
    ...
    public static void getpid(Environment env) {
        ...
        env.register.set('0', new BitVec(ret, true));
...
//    }
    ...
    // EMPTY STUB: int pthread_cond_wait(pthread_cond_t *cond, pthread_mutex_t *mutex)
    public static void pthread_cond_wait(Environment env) {}
    public static void pthread_join(Environment env) {}
    public static void pthread_mutex_unlock(Environment env) {}
    public static void pthread_create(Environment env) {}
    public static void pthread_mutex_lock(Environment env) {}

    // ANDROID STUB
    public static void __android_log_print(Environment env) {
        boolean isSink = false;
        char markedC = 'x';
        for (char c = '0'; c <= '3'; c++) {
            if (env.register.get(c).taint) {
                markedC = c;
            }
            isSink = isSink || env.register.get(c).taint;
        }
        if (isSink) {
            Logs.infoLn("SINK: __android_log_print");
            BitVec prev = env.register.get(markedC).getPrev();
        }
    }
    public static void _ZN7_JNIEnv9NewObjectEP7_jclassP10_jmethodIDz(Environment env) {
        env.memory.put(env.register.get('3'), env.register.get('1'));
        Memory.set(env.register.get('3'), env.register.get('1'));
        System.out.println("Create Object at " + env.register.get('3').getSym());
        env.register.set('0', env.register.get('3'));
    }
    private static boolean isSource(GhidraRun.AssemblyInstruction ghidraInst) {
        if (ghidraInst == null) return false;
        return Taint.isSource(ghidraInst.getRefString());
    }
    public static void _ZN7_JNIEnv16CallObjectMethodEP8_jobjectP10_jmethodIDz(Environment env) {
        if (env.register.get('0').taint || env.register.get('1').taint || env.register.get('2').taint) {
            System.out.println("SOURCE: Call " + env.markedSource);
            env.register.get('0').taint();
            System.out.println("+++ Taint r0: " + env.register.get('0').toString());
            env.register.get('1').clear();
            env.register.get('2').clear();
        }
    }
    //"0x2b4","jobject _func_409(JNIEnv * env\, jobjectArray array\, jsize index) *","_func_409 *","GetObjectArrayElement"
    public static void JNIEnv_GetObjectArrayElement(Environment env) {
        BitVec t0 = env.register.get('1'); // array
        BitVec t1 = env.register.get('2'); // index
        String start = t0.getSym();
        long index = Arithmetic.hexToInt(t1.getSym());
        if (index > -1 ) {
            for (int i = 0; i < index; i++) {
                start = JNIEnvHelper.addHex(start, "#x00000004");
            }
        }
        BitVec res = Memory.get(start); // = array[index]
        env.register.set('0', res);
    }
}